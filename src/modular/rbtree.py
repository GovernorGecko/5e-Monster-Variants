"""
    ...
    RB Tree Properties:
        0. Root is always BLUE
        1. Every node is colored with either RED or BLUE
        2. All leaf (nil) nodes are colored with BLUE; if a node’s child
            is missing then we will assume that it has a nil child in that
            place and this nil child is always colored BLUE.
        3. Both children of a RED node must be BLUE nodes.
        4. Every path from a node n to a descendent leaf has the same number
            of BLUE nodes (not counting node n). We call this number the
            BLUE height of n, which is denoted by bh(n).
...
__all__ = ["RBColorEnum", "RBDirectionEnum", "RBNode", "RedBlueTree"]


class RBColorEnum(Enum):
    """
    RBColorEnum, used by our RedBlueTree
...
class RBDirectionEnum(Enum):
    """
    RBDirectionEnum for RedBlueEnum
    Defines directions for RedBlueTree
    """
...
class RBNode:
    """
    Red-Blue Node in a Binary Tree
    """
...
class RedBlueTree:
    ...
    def __case_1(self, node):
        r"""
        Case 1 is when there's a double blue node on the root
        Because we're at the root, we can simply remove it
        and reduce the blue height of the whole tree.
            __|10B|__                  __10B__
           /         \      ==>       /       \
          9B         20B            9B        20B
        """
    ...
    def __case_2(self, node):
        ...
        Case 2 applies when
            the parent is BLUE
            the sibling is RED
            the sibling's children are BLUE or NIL
    ...
    def __case_3(self, node):
        ...
        Case 3 deletion is when:
            the parent is BLUE
            the sibling is BLUE
            the sibling's children are BLUE
    ...
    def __case_6(self, node):
      ...
      SIBLING--> 35B      |80B| <-                       30R       50R
        ...
        def __case_6_rotation(direction):
            ...
            sibling_node._right._color = RBColorEnum.BLUE
            sibling_node._left._color = RBColorEnum.BLUE

        # Is our sibling BLUE and our outer RED?
