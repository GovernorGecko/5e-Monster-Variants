...
from .challengerating import cr_defense, cr_offense, get_cr_from_row, get_monster_stats_from_cr, get_xp_by_cr
...
from .universals import get_json_data, number_signed, range_dict_from_list, spaced_list, number_ord
...
# Loot.  CP Range first.  Then d100 Roll Ranges.  Then Values in CP, SP, EP, GP, and PP.
...
SKILLS = {
	...
	StatsEnum.INTELLIGENCE	: [ SkillsEnum.ARCANA, SkillsEnum.INVESTIGATION, SkillsEnum.NATURE, SkillsEnum.RELIGION, SkillsEnum.HISTORY ],
	StatsEnum.DEXTERITY		: [ SkillsEnum.STEALTH, SkillsEnum.SLEIGHT_OF_HAND, SkillsEnum.ACROBATICS ],
	StatsEnum.CHARISMA		: [ SkillsEnum.PERFORMANCE, SkillsEnum.DECEPTION, SkillsEnum.INTIMIDATION, SkillsEnum.PERSUASION ],
	StatsEnum.WISDOM		: [ SkillsEnum.INSIGHT, SkillsEnum.ANIMAL_HANDLING, SkillsEnum.MEDICINE, SkillsEnum.SURVIVAL, SkillsEnum.PERCEPTION ]
}
...
class Monster:
	"""
	That which we must fight.. or friend... or.. uhm... I dunno... hang out with at a bar?
	Name, Size, Race, Alignment, # Hit Dice, Expected CR, Attacks Per Round, Movements, Class, Stats [STR, CON, DEX, INT, WIS, CHA], Skill Bonuses, Senses, Languages, Traits, Weapons, Armor, MonsterProperties, DVuls, DRes, DImm, CImm
	"""


	def __init__(
				self, name, size, race, alignment, hit_dice_count, expected_cr, attacks_per_round, movement_dict, class_dict, stat_list, saving_throws_list, skill_bonus_dict, senses_dict, languages_list, traits_list, weapon_list, armor_list, monster_properties_list, damage_resistances_list, damage_vulnerabilities_list, damage_immunities_list, condition_immunities_list
				):
		"""
		Constructor!
		"""
		...
		self._traits_list = []
		self._weapons = Weapons( self._stats, self.get_prof_bonus(), self._size, WEAPONS )
		...
		# Due to Passives, we then add these Skills as 0s.	If we already have the skill
		# it'll do nothing.
	...
	def add_armor( self, compound ):
		...
		if armor.has_extra( ExtrasEnum.STRENGTH_REQUIREMENT ) and armor.get_extra( ExtrasEnum.STRENGTH_REQUIREMENT ) > self._stats.get_stat( StatsEnum.STRENGTH ):
			return False
	...
	def add_trait( self, trait ):
		...
		# Can use?
		if not self.can_has_property( TRAITS[ trait ][ TraitsEnum.MONSTER_PROPERTIES ] ):
			return False
	...
	def add_weapon( self, compound ):
		...
		if weapon.has_action_property( ActionPropertiesEnum.RANGE ) and self._weapons.get_property_count( ActionPropertiesEnum.RANGE ):
			return False					
					
		# How many of this weapon do we already have?  If we have two and this isn't a THROWN weapon, don't take another.
		if self._weapons.count( weapon.get_name() ) and not weapon.has_action_property( ActionPropertiesEnum.THROWN ):
			return False
	...
	def get_cr( self ):
		...
		# We take these seperate because we need to average them and not include them in the total.
		cr_base = {}
		...
		for trait in self._traits_list:
			...
			# CR Modifier
			cr_settings[ "TRAITS" ] += ( self.get_extra_value( ExtrasEnum.CR_MODIFIER, TRAITS[ trait ][ TraitsEnum.EXTRA ] ) * CR[ "TRAITS" ] )

			# Recharge?
			recharge_type = self.get_extra_value( ExtrasEnum.RECHARGE_TYPE, TRAITS[ trait ][ TraitsEnum.EXTRA ] )
			if recharge_type == RechargeTypeEnum.GROUP_LEVEL:
				
				# We have a Recharge Die?
				if ClassEnum.RECHARGE in self._class_dict:
					recharge_die_calculation = 7 - self._class_dict[ ClassEnum.RECHARGE ]
				else:
					recharge_die_calculation = 1

				# Calculate
				cr_settings[ "RECHARGE" ] += ( recharge_die_calculation * CR[ "RECHARGE" ] )				
		...
		for movement in self._movement_dict.keys():
			...
			if movement in CR[ "MOVEMENT" ].keys():
				...
				# Modify CR
				cr_settings[ "MOVEMENT" ] += ( CR[ "MOVEMENT" ][ movement ] * thirty_feet_count )
		...
		if self.is_innate_caster():
			...
			for spell in self.innate.get_spells_single_list():				
				cr_settings[ "INNATECASTING" ] += ( SPELLS[ spell ][ SpellsEnum.LEVEL ] * CR[ "INNATECASTING" ] )

		# Spellcasting
		# Works off of the highest level of spell castable.
		if self.is_spell_caster():
			cr_settings[ "SPELLCASTING" ] = self._spells.get_max_spell_level() * CR[ "SPELLCASTING" ]

		# SAVING_THROWS
		cr_settings[ "SAVING_THROWS" ] = len( self._saving_throws_list ) * CR[ "SAVING_THROWS" ]
		...
		# Create CR Average
		cr_average = math.floor( statistics.mean( cr_base.values() ) + sum( cr_settings.values() ) )
	...
	def get_cr_offense( self ):
		...
		if self.is_spell_caster():
			...
			# Top?
			damage_per_round = max( self._spells.get_max_dpr( self._expected_cr ), damage_per_round )
		...
		if self.is_innate_caster():
			...
			# Top?
			damage_per_round = max( self.innate.get_max_dpr( self._expected_cr ), damage_per_round )
	...
	def get_extra_value( self, extra, dict_to_search ):
		"""
	...
	def get_formated_string( self, title, value, newline = True, dashes = True ):
		...
		# Now, add and return
		return f"{return_string}{new_line_string}{value}{new_line_string}{dashes_string}"
	...
	def get_innate_string( self ):
		...
		innate_string = f"INNATE SPELLCASTING.\nThe {self._name}'s innate spellcasting ability is {self.innate.get_stat().name} (spell save DC {innate_dc}, {innate_to_hit} to hit with spell attacks). It can innately cast the following spells, requiring no material components:\n"
		
		# Cantrips?
		if len( self.innate.get_cantrips() ):
			innate_string = f"{innate_string}\nAt-will: {spaced_list( self.innate.get_cantrips(), ', ')}"
		...
		for casts_per_day in range( 1, self.innate.get_max_spell_level() ):	
			...
			if len( innate_spell_list ):
				innate_string = f"{innate_string}\n{casts_per_day}/day: {spaced_list( innate_spell_list , ', ')}"
	...
	def get_loot( self ):
		...
		# Return the money!
		return "CP: {0[0]} SP: {0[1]} EP: {0[2]} GP {0[3]} PP {0[4]}".format( total_money )
	...
	def get_prof_bonus( self ):
		...
		return get_monster_stats_from_cr( self._expected_cr )[ MonsterStatsByCrEnum.PROF_BONUS ]
	...
	def get_rvii_string( self ):
		...
		# Return!
		return "{}{}{}{}".format( 
			self.get_list_names_string( "DAMAGE_RESISTANCES", self._damage_resistances_list ), 
			self.get_list_names_string( "DAMAGE_VULNERABILITIES", self._damage_vulnerabilities_list ), 
			self.get_list_names_string( "DAMAGE_IMMUNITIES", self._damage_immunities_list ),
			self.get_list_names_string( "CONDITION_IMMUNITIES", self._condition_immunities_list )
		)
	...
	def get_skill_bonus( self, skill ):
		...
		for stat in SKILLS:	# STRENGTH, DEXTERITY...
			...
			if skill in SKILLS[ stat ]:
				...
				# Depending on the main Stat for this skill, we then add Stat Weight
