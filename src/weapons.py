...
class Weapon():
    ...
    def __init__(self, name, scale, stats, data, size_count_bonus):
        ...
        # Create the Dice 
        self._dice = self._data[WeaponsEnum.DAMAGE_DICE].get_scaled(self._scale)
...
class Weapons():
    ...
    def create(self, compound, add = False):
        ...
        # Create the weapon.
        weapon = Weapon(name, scale, self._stats, self._data, self._size_count_bonus)
        
        # Do we have Improvised Dice?
        if weapon.has_extra(ExtrasEnum.IMPROVISED):
            i_name, i_scale = get_actual_from_compound(weapon.get_extra(ExtrasEnum.IMPROVISED), self._data)
            weapon._improvised = Weapon(i_name, i_scale, self._stats, self._data, self._size_count_bonus)

        # Do we have Versatile Dice?
        if weapon.has_extra(ExtrasEnum.VERSATILE):
            v_name, v_scale = get_actual_from_compound(weapon.get_extra(ExtrasEnum.VERSATILE), self._data)
            weapon._versatile = Weapon(v_name, v_scale, self._stats, self._data, self._size_count_bonus)
    ...
    def get_dpr_max(self):
        ...
        for weapon in self._weapon_list:
            ...
            # Damage Per Round
            damage_per_round = max(weapon._dice.get_average() + weapon.get_stat_bonus(), damage_per_round)
    ...
    def get_property_count(self, action_property, included = True):
        ...
        for weapon in self._weapon_list:
            ...
            # We find it?
            action_property_found = action_property in weapon._data[WeaponsEnum.ACTION_PROPERTIES]
    ...
    def print(self):
        ...
        for weapon in self._weapon_list:
            ...
            # To Hit
            to_hit_string = number_signed(self._prof_bonus + weapon.get_stat_bonus())
            
            # Get Damage
            hit_damage = f"{weapon._dice}{number_signed(weapon.get_stat_bonus())}"
            ...
            if weapon.has_action_property(ActionPropertiesEnum.RANGE):
                reach = f"RANGE: {weapon._data[WeaponsEnum.EXTRA][ExtrasEnum.RANGE]}FT"
                weapon_attack_type = "Ranged"
            
            # Thrown
            if weapon.has_action_property(ActionPropertiesEnum.THROWN):
                reach = f"{reach} (THROWN  {weapon._data[WeaponsEnum.EXTRA][ExtrasEnum.RANGE]}FT)"
                            
            # Versatile
            if weapon.has_extra(ExtrasEnum.VERSATILE):
                hit_damage = f"{hit_damage} ({weapon._versatile._dice}{number_signed(weapon.get_stat_bonus())})"
                
            # Improvised
            if weapon.has_extra(ExtrasEnum.IMPROVISED):
                hit_damage = f"{hit_damage} ({weapon._improvised._dice}{number_signed(weapon.get_stat_bonus())} IMPROVISED MELEE)"
            ...
            if weapon.has_monster_property(MonsterPropertiesEnum.NATURAL_WEAPON):
                weapon_composition_type = "Natural"
            
            # Build the data.
            w_data = f"{weapon._name}. {weapon_attack_type} {weapon_composition_type} Attack: {to_hit_string} TO HIT, {reach}., HIT: {hit_damage}{extras_light}{damage_type} DAMAGE"
